...
class WorkOrdersWN extends Processor {
  constructor(options) {
    super(options);
    // Processor Specific configuration
    this.recordsEntity = 'com.aleron.monitor.WorkOrders_WN';
    this.LOG = cds.log('Monitor.Processor-WorkOrdersWN');
  ...
  _getColumnsForFetch(sEntity) {
    const mEntityColumns = {
      'com.aleron.monitor.WorkOrders_WN': [
        ...['ID', 'file_ID', 'processLevel_code', 'valid',], // mandatory columns
        ...['salesOffice', 'salesDocumentType', 'salesDocItem',], // sales data columns
        ...['contractNo', 'companyCode', 'soldToParty'], // sales contract
  ...
  async validateRecords(sProcessCode, bBreakExecution) {
    ...
    for (const record of this.records) {
      ...
      if (record.country_code) {
        aCountries.push(record.country_code);
      }
    ...
    const [
      ...
      { reason: anySupplierErr, value: aSuppliers },
      ...
      this.salesContractAPI.executeQuery(
        SELECT.from('SalesContract')
          .columns(['SalesContract', 'SalesOrganization', 'DistributionChannel', 'SoldToParty'])
      ...
      this.businesPartnerAPI.executeQuery(
        SELECT.from('A_Supplier')
          .columns(['Supplier'])
          .where({ Supplier: { in: aRemitVendors } }),
      ),
      ...
      this.salesOrderAPI.executeQuery(
        SELECT.from('A_SalesOrderItem')
          ...
          .where({
            YY1_WNWorkOrder_SD_SDI: { in: aWNWorkOrders },
            SalesOrderItem: '10'
          })
      )
    ]);

    // Combine SalesContract results (primary + fallback)
    ...
    if (mDistChannelMap.size) {
      for (const [id, { distributionChannel }] of mDistChannelMap.entries()) {
        // update this.records 
  ...
  async processSalesOrder(sProcessCode, bBreakExecution) {
      ...
      const [
        ...
        this.billingTypeAPI.executeQuery(
          SELECT.from('YY1_BILLINGTYPE')
            ...
            .where({
              SO_order_Type: { in: [...new Set(asalesDocumentTypeWhere)] }
            })
        )
      ]);
    ...
    try {
      const [
        ...
        { reason: anyProductErr, value: aProducts },
      ] = await Promise.allSettled([
        this.businesPartnerAPI.executeQuery(
        ...
        this.productAPI.executeQuery(
          SELECT.from('A_ProductSalesDelivery')
            ...
            .where({
              Product: { in: aMaterialNoWhere },
              ProductSalesOrg: { in: aSalesOrgWhere },
    ...
    try {
      const [
        { reason: anyCustEDIPartnerErr, value: aCustEDIPartners },
        { reason: anyEdiShiftEmpSubGrpErr, value: aEdiShiftEmpSubGrps },
        { reason: anyEdiShiftCAErr, value: aEdiShiftCAs },
      ] = await Promise.allSettled([
        ...
        SELECT.from('com.aleron.monitor.ediShiftDiffForCanada')
          ...
          .where({
            customerNo: { in: aCustomerEDIPartnerWhere },
            region: { in: aRegionWhere },
            materialNo: { in: aMaterialNoWhere },
          }),
      ]);
    ...
    // Step 4: Create SalesOrders in S/4HANA via ODatas
  ...
  _prepareDataForSalesOrderCreate({
    ...
    const oReturnData = {
      ...
      YY1_CustomSalesOrder_SDH: record.salesDocumentType === 'MS' ? 'ZWMS' : record.salesDocumentType === 'SC' ? 'ZWSC' : 'ZW' + records.salesDocumentType,  // for IC type there is no condition for now
  ...
  _preparePartnerFunctions({ record, businessPartnerMap, vendorRemit, supplierAcc, personalNO, taxCode }) {
    ...
    if (!oEmailContact) {
      ...
      aPartnerFunctions.push({
        PartnerFunction: 'Z5',
        ContactPerson: oEmailContact.ContactPerson, //'21',  // Mapping needs to be done dynamic
        to_Address: [oAddr],
      });
    }
...
if (Object.keys(foundFields).length > 0) {
  ...
  //     // Z30: no YY target provided -> ignore
  ...
  //     // Z36: no YY target provided -> ignore
  //     Z37: 'YY237_CUST_POSITION',
  //     // Z38: no YY target provided in list -> ignore
  //     Z39: 'CUST_CATERGORY_CODE2',       // non-YY, VC2
  //     Z40: 'YY6_SC_LINE_ITEM_NUMBER',   // VC1
  //     // Z41: no mapped YY in list -> ignore
  ...
  //   // Which targets belong to VC1 vs VC2 (EDMX-based check)
  ...
  //   // 1) Filter failed; 2) Build payloads
  //   let aPayloadsSalesVCData = records
  //     .filter((record) => !aFailedRecordIDs.includes(record.ID))
  //     .map((record) => {
  //       const oMapEntry = mPayloadMap.get(record.ID);
  ...
  //       // Translate Zxx -> target property and bucket into VC1/VC2
  //       const oCustFieldResult = aCustomerfieldEntry.reduce(
  //         (acc, entry) => {
  //           const z = (entry.customerFieldName || '').trim();
  //           const target = Z_MAP[z];
  //           if (!target) return acc; // unknown/unused Z -> skip

  //           // Decide which VC entity gets it
  //           if (VC1_PROPS.has(target)) {
  //             acc.VC1Fields[target] = entry.customerFieldValue;
  //           } else {
  //             // default to VC2 (covers VC2_PROPS and non-YY VC2 like ACCELERATED_FEE_DISC_VEN)
  ...
  //       // keep your existing mapping AS-IS (only Z mapping above is new)
  //       let shift = 1;
...
Z38:{target:'CUST_COMMODITY_CODE2',vc:2},
    ...
    const prepared = records
      ...
      .map((record) => {
        ...
        // VC1 payload (kept same fields as your original; now normalized)
        const salesVC1 = {
        ...
        // VC2 base (kept your mapping where applicable) → then apply Z overrides
        const baseVC2 = {
          ...
          // Additional VC2 attributes used across interfaces (kept space for them if present on record)
          ...
          // Weekly Clock Fee (base; Z08 overrides if present)
          YY144_WEEKLY_CLOCK_FEE: fmtDecimal(record.custWkTimeFee),

          // Optional daily/salary fields if you later add them for I1 (kept compatible with validation)
        ...
        // Validate formats (collect but do not hard-stop)
        const localErrs = [];
        ...
        // Debug echo (kept)
        try {
          LOG.info(`[VC] Constructed payloads recID=${record.ID}`, {
            recID: record.ID,
            salesOrder: String(oMapEntry.salesOrder),
            vc1Payload: JSON.stringify(salesVC1, null, 2),
            vc2Payload: JSON.stringify(salesVC2, null, 2),
    ...
    for (const row of prepared) {
      ...
      // VC2 insert/skip with try/catch
      try {
        if (!row.vcData2UUID) {
          insertedSalesVCData2 = await SalesVCData_2.executeQuery(
      ...
      // POST log (kept)
      try {
        LOG.info(`[VC] POST-INSERT result recID=${recID}`, {
          recID,
          vc1Result: JSON.stringify(insertedSalesVCData1 || {}, null, 2),
          vc2Result: JSON.stringify(insertedSalesVCData2 || {}, null, 2),
        });
  ...
  async updateInfotype(sProcessCode, bBreakExecution) {
    ...
    for (const record of aRecordsForProcessing) {
      try {
        ...
        // if (record.ssn) filters.SSN = record.ssn;
        // if (record.firstName) filters.push({ FirstName: record.firstName });
        // if (record.lastName) filters.push({ Name: record.lastName });
        // if (record.middleName) filters.push({ MiddleName: record.middleName });
        // filters.push({ END_DATE: { '<=' : new Date().toISOString() } });
